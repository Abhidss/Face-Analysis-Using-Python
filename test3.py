...
def detect_age_gender(frame, face_cascade, age_net, gender_net, age_labels, gender_labels):
    """
    Detect faces and predict age and gender for each face
    Returns the modified frame with age and gender labels
    """
    ...
    for (x, y, w, h) in faces:
        ...
        # Save results for later use if needed
...
def detect_face_shape(frame, dlib_detector, predictor):
    """
    Detect faces and determine face shape for each face
    Returns the modified frame with face shape labels and landmarks
    """
    ...
    for face in dlib_faces:
        ...
        # Draw rectangle (yellow to differentiate from age/gender detection)
...
def main():
    ...
    print("Face analysis started. Press 'q' to quit.")
    ...
    while True:
        ...
        # Create a copy of the frame for each detection method
        display_frame = frame.copy()
        
        # Step 1: Age and Gender Detection
        display_frame, _ = detect_age_gender(
            display_frame, face_cascade, age_net, gender_net, age_labels, gender_labels
        )
        
        # Step 2: Face Shape Analysis on the same frame
        display_frame = detect_face_shape(display_frame, dlib_detector, predictor)
        
        # Display the frame with all analyses
